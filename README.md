...
Use `npm link` if you want `taskctl` on your shell path. Set `CODEX_TASKBOARD_URL` to point the CLI at another local or LAN service. Cloud deployments are configured through the **loopback companion** (device-local loopback service for auth, project paths, and complete Codex thread bindings—not a chat persona) with `taskctl cloud login`; device-local path and binding options are rejected for a direct Cloud URL.
...
For a reverse tunnel that connects to the local listener, set `CODEX_TASKBOARD_TRUSTED_ORIGINS` to the tunnel's public HTTPS origin, for example `https://board.example.test`. Multiple origins are comma-separated. The variable cannot be empty, and duplicate origins (including normalized forms such as a trailing slash or default HTTPS port) are rejected at startup. Entries must otherwise be exact HTTPS origins; paths, queries, fragments, credentials, and wildcards are rejected. A reverse proxy or tunnel must preserve a loopback socket connection and may preserve the exact configured public authority in `Host` or rewrite it to a local/private host. The browser's HTTPS `Origin`, when present, is checked independently, and forwarded headers are not used for either decision. Requests identified by either the exact configured public `Host` or `Origin` can use ordinary Taskboard HTTP and realtime endpoints, but device-local capability routes and metadata stay unavailable even though the tunnel socket is loopback. Direct local or private-LAN requests keep their existing capabilities.
